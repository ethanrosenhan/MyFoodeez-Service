...
// Social sign-in columns. google_id / apple_id hold the provider's stable
// 'sub'; auth_provider records how the account was created. Partial unique
// indexes (WHERE ... IS NOT NULL) let many rows keep a NULL provider id while
// still guaranteeing one account per linked Google/Apple identity.
const addOAuthColumns = async () => {
    await sequelize.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS auth_provider VARCHAR(255)');
    await sequelize.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS google_id VARCHAR(255)');
    await sequelize.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS apple_id VARCHAR(255)');
    await sequelize.query('CREATE UNIQUE INDEX IF NOT EXISTS users_google_id_unique ON users (google_id) WHERE google_id IS NOT NULL');
    await sequelize.query('CREATE UNIQUE INDEX IF NOT EXISTS users_apple_id_unique ON users (apple_id) WHERE apple_id IS NOT NULL');
    return { ok: true };
};

// Video support. post_media is created by sync() on fresh installs; these are
// belt-and-suspenders for existing DBs. Stored as a separate table (not BLOB
// columns) because the bytes live in Cloudinary — we only persist URLs.
const addPostMediaIndexes = async () => {
    await sequelize.query('CREATE INDEX IF NOT EXISTS post_media_post_id_idx ON post_media (post_id)');
    return { ok: true };
};
...
const addPostReactionColumn = async () => {
    await sequelize.query("ALTER TABLE post_stars ADD COLUMN IF NOT EXISTS reaction VARCHAR(255) NOT NULL DEFAULT 'heart'");
    return { ok: true };
};

const addPostPreviewStyleColumn = async () => {
    await sequelize.query("ALTER TABLE posts ADD COLUMN IF NOT EXISTS preview_style VARCHAR(255) NOT NULL DEFAULT 'cover-center'");
    return { ok: true };
};
...
// Profile visibility. NOT NULL DEFAULT true backfills every existing user to
// public, matching the chosen default so the browse-profiles feature works
// for accounts created before this column existed.
const addUserIsPublicColumn = async () => {
    await sequelize.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS is_public BOOLEAN NOT NULL DEFAULT true');
    return { ok: true };
};

// Admin/moderator flag. Adds the column (default false) and grants admin to
// the owner account so there's a moderator from day one. Granting more admins
// later is a one-line UPDATE (or scripts/grant-admin.js) — no code change.
const addUserIsAdminColumn = async () => {
    await sequelize.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS is_admin BOOLEAN NOT NULL DEFAULT false');
    const [, meta] = await sequelize.query(
        "UPDATE users SET is_admin = true WHERE LOWER(email) = LOWER('eprosenhan@gmail.com')"
    );
    const granted = meta?.rowCount ?? 0;
    console.log(`addUserIsAdminColumn: granted admin to ${granted} matching account(s)`);
    return { granted };
};
...
const runStartupMigrations = async () => {
    try {
        ...
        await runOnce('migration:add_oauth_columns_v1', addOAuthColumns);
        await runOnce('migration:add_post_media_indexes_v1', addPostMediaIndexes);
        ...
        await runOnce('migration:add_post_reaction_column_v1', addPostReactionColumn);
        await runOnce('migration:add_post_preview_style_column_v1', addPostPreviewStyleColumn);
        ...
        await runOnce('migration:add_user_is_public_column_v1', addUserIsPublicColumn);
        await runOnce('migration:add_user_is_admin_column_v1', addUserIsAdminColumn);
    } catch (error) {
