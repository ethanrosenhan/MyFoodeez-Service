...
//   - blocks and content reports tied to their account or authored posts
...
export const deleteUserAndAllData = async (userId) => {
    ...
    return sequelize.transaction(async (transaction) => {
        ...
        if (postIds.length > 0) {
            ...
            await models.content_report.destroy({ where: { post_id: { [Op.in]: postIds } }, transaction });
        ...
        await models.content_report.destroy({ where: { reporter_user_id: id }, transaction });
        await models.user_block.destroy({
            where: { [Op.or]: [{ blocker_user_id: id }, { blocked_user_id: id }] },
            transaction
        });
